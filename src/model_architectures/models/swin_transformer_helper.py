...
class Mlp(nn.Module):
    def __init__(self, in_channals, hidden_features, drop=0.):
        super().__init__()
        self.fc1 = nn.Linear(in_channals, hidden_features)
        self.act = nn.GELU()
        self.fc2 = nn.Linear(hidden_features, in_channals)
...
def window_partition(x, window_size):
    ...
    Args:
        x: (B, H, W, C)
        window_size (list): window size, window_size
    Returns:
...
class UpSample(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reorganize x to increase the resolution: simply change the order and upsample from (H, W) to (2H, 2W)
        # Reshape x to facilitate upsampling.
        x = x.view(x.shape[0], self.input_resolution[0], self.input_resolution[1], 2, 2, x.shape[-1]//4)
        # Change the order of x
        x = x.permute(0,1,3,2,4,5).contiguous() 
        x = x.view(x.shape[0],self.input_resolution[0]*2,self.input_resolution[1]*2,x.shape[-1])
